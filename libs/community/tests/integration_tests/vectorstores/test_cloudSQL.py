...
async def test_CloudSQLVectorStore_with_metadatas() -> None:
    ...
    output = await docsearch.similarity_search("foo", k=1)
...
async def test_CloudSQLVectorStore_reindex() -> None:
    ...
    db = CloudSQLVectorStore(
        ...
        embedding_service=FakeEmbeddingsWithAdaDimension(),
        index=index)
    output = await db.areindex(index)
    assert output == None

async def test_CloudSQLVectorStore_dropindex() -> None:
